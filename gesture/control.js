...
// === Real-Time Neon Gesture Trail + Double Tap + Long Press ===
(function(){
  ...
  window.addEventListener("touchstart",(e)=>{
    ...
    if(y < windowHeight*(1-bottomArea)) return; // ignore upper 60%
  ...
  window.addEventListener("touchend",(e)=>{
    ...
    volHighBtn.click(); // release action

    const diffX = touch.clientX - touchStartX;

    // Swipe detection
    if(Math.abs(diffX) > 50){
      if(diffX<0) nextBtn.click();  // left → next
      else prevBtn.click();         // right → prev
    } else {
